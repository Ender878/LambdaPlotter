...
#ifndef _WIN32
LP::Serial::Serial(const char* port, const size_t baud)
{
    termios tty{};
...
std::vector<std::string> LP::Serial::read_system_ports()
{
    ...
    for (const auto& entry : std::filesystem::directory_iterator("/dev/"))
    {
        ...
        if (auto entry_str = entry.path().string();
            entry_str.find("ttyACM") != std::string::npos || entry_str.find("ttyUSB") != std::string::npos)
        {
...
bool LP::Serial::read(std::vector<char>& buf) const
{
    ...
    const long n = ::read(serial_port_fd, buf.data(), buf.size());
