...
void LP::PlotView::render_plot(Telemetry &tel, app_state_t app_state, int pos_x, int pos_y,
                               int width, int height) {
    std::lock_guard<std::mutex> lock(tel.get_data_mtx());

    std::vector<double> *times = (plot_style.time_style == DATETIME) ? tel.get_unix_timestamps()
                                                                     : tel.get_elapsed_timestamps();
    ...
    if (ImGui::Begin("##plot",
        ...
        if (!data->empty()) {
            if (ImPlot::BeginPlot(
                ...
                double window_start =
                    (time_window != 0) ? std::max(first_time, last_time - time_window) : first_time;
                ...
                plot_style.limits = {
                    limits.Min().x, limits.Max().x, limits.Min().y, limits.Max().y};
                ...
                for (auto &stream : *data) {
                    ...
                    if (plot_attributes[data_id].show) {
                        ...
                        if (channel.offset != channel.prev_offset ||
                            channel.scale != channel.prev_scale) {
                            channel.values_transformed.clear();
                            for (size_t i = 0; i < channel.values.size(); i++) {
                                channel.values_transformed.push_back(
                                    (channel.values[i] * channel.scale) + channel.offset);
                            }
                        }
                        ...
                        plot_functions[plot_attributes[data_id].combobox_func_index].func(
                            label.c_str(),
                            times->data(),
    ...
    if (!tel.is_empty()) {
        if (ImGui::BeginTable("##TelTable", 4)) {
            ...
            for (auto &data : *tel.get_data()) {
                ...
                float selected_color[4] = {plot_attributes[data_id].color.x,
                                           plot_attributes[data_id].color.y,
                                           plot_attributes[data_id].color.z,
                                           plot_attributes[data_id].color.w};
                ImGui::ColorEdit4("##Color", selected_color, ImGuiColorEditFlags_NoInputs);
                plot_attributes[data_id].color = {
                ...
                std::strncpy(
                    display_name_buffer, data.second.name.c_str(), sizeof(display_name_buffer) - 1);
                ...
                std::string bt_id =
                    std::format("{}", plot_attributes[data_id].show ? "HIDE" : "SHOW");
...
void LP::PlotView::render_data_format(Telemetry &tel, app_state_t app_state) {
    ...
    bool same_str_channel_sep =
        ...
        ((tel.frame_format.named)
             ? std::strcmp(tel.frame_format.channel_sep, tel.frame_format.name_sep) == 0
             : 0);

    bool same_str_frame_end =
        std::strcmp(tel.frame_format.frame_end, tel.frame_format.channel_sep) == 0 ||
        ((tel.frame_format.named)
             ? std::strcmp(tel.frame_format.frame_end, tel.frame_format.name_sep) == 0
             : 0);

    bool same_str_name_sep =
        (tel.frame_format.named)
    ...
    if (ImGui::BeginTable("##format_table", 2)) {
        ...
        ImGui::InputText(
            "##channel_sep", tel.frame_format.channel_sep, sizeof(tel.frame_format.channel_sep));
        ...
        ImGui::InputText(
            "##frame_end", tel.frame_format.frame_end, sizeof(tel.frame_format.frame_end));
        ...
        if (tel.frame_format.named) {
            ...
            ImGui::InputText(
                "##name_sep", tel.frame_format.name_sep, sizeof(tel.frame_format.name_sep));
    ...
    float prev_size =
        (2 * ImGui::CalcTextSize(tel.frame_format.channel_sep).x + 3 * ImGui::CalcTextSize("X").x +
         ImGui::CalcTextSize(tel.frame_format.frame_end).x);

    if (tel.frame_format.named) {
        prev_size +=
            3 * ImGui::CalcTextSize("N").x + 3 * ImGui::CalcTextSize(tel.frame_format.name_sep).x;
    }

    ImGui::SetCursorPosX(ImGui::GetCursorPosX() +
                         (ImGui::GetContentRegionAvail().x - prev_size) * 0.5);
    ...
    if (same_str_channel_sep || same_str_frame_end || same_str_name_sep) {
        ImGui::TextColored(ImVec4(1.0f, 0.0f, 0.0f, 1.0f),
                           "Duplicate tokens, Data won't be parsed correctly!");
    }
