...
void LP::ToolBar::update_serial_ports(const std::vector<std::string>& serial_ports)
{
    if (refresh_button)
    {
        ...
        else if (!serial_ports.empty() && combobox_port_index.has_value())
        {
            ...
            // the pointed value exists in the list and reassign the correct
            // index if the pointed element no longer exist, set the index to
            // null
            if (std::ranges::any_of(serial_ports.begin(),
                                    serial_ports.end(),
                                    [this](const std::string& v) { return v == current_port; }))
            {
                combobox_port_index = std::distance(
                    serial_ports.begin(), std::ranges::find(serial_ports.begin(), serial_ports.end(), current_port));
            }
            else
            {
                combobox_port_index = std::nullopt;
    ...
    if (ImGui::BeginTable("##toolbar_layout", 2))
    {
        ...
        if (ImGui::BeginCombo("##baud", selected_baud))
        {
            for (size_t i = 0; i < LP::baud_rates.size(); i++)
            {
                ...
                if (const bool selected = combobox_baud_index == i; ImGui::Selectable(LP::baud_rates[i].str, selected))
                {
